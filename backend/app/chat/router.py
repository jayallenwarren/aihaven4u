...
EXPLICIT_PATTERNS = [
    ...
    r"\bblowjob\b", r"\bsquirt\b", r"\bpee\b", r"\bfart\b", r"\bbj\b",
    r"\bpussy\b", r"\banal\b", r"\bdeep throat\b",
]
...
def route_turn(user_text: str, session_state: Dict[str, Any]) -> Tuple[str, str]:
    if _matches_any(user_text, CRISIS_PATTERNS):
        return ("crisis",
                "I’m really sorry you’re feeling this way. If you’re in immediate danger, contact emergency services or a crisis line.")
    ...
    if wants_explicit:
        if not session_state.get("adult_verified", False):
            return ("need_explicit_consent",
                    "Before we go further, I need to confirm you’re 18+ and that you want explicit adult conversation. Are you 18 or older, and do you want to opt into Explicit Mode?")
        if not session_state.get("explicit_consented", False):
            return ("need_explicit_consent",
                    "I can do explicit adult conversation only with your clear opt-in. Do you want to enter Explicit Mode? You can say yes or no.")
